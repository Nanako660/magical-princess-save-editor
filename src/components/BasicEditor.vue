<template>
  <section class="editor-section">
    ...
    <div class="card-grid basic-grid">
        ...
        <n-card title="时间与资源" size="small" class="editor-panel">
          <n-form :model="status" label-placement="top">
            <n-grid :cols="24" :x-gap="16" :y-gap="10">
              ...
              <n-form-item-gi :span="12" label="行动力">
                <n-input-number v-model:value="status.activePower" :min="0" :max="99" />
              </n-form-item-gi>
              <n-form-item-gi :span="12" label="最大行动力">
      ...
      <div class="card-grid-item card-grid-item--third">
        <n-card title="倾向与成长" size="small" class="editor-panel">
          <n-form :model="status" label-placement="top">
      ...
      <div class="card-grid-item card-grid-item--third">
        <n-card title="身份与关系" size="small" class="editor-panel">
          <n-form :model="status" label-placement="top">
            <n-grid :cols="24" :x-gap="14" :y-gap="10">
...
.editor-panel :deep(.n-base-selection) {
  max-width: 280px;
}

.editor-panel :deep(.n-input-group) {
  width: 100%;
  max-width: 230px;
}

.editor-panel :deep(.n-input-group .n-input-number) {
  width: 100% !important;
  min-width: 0;
}

.basic-grid .card-grid-item:last-child .editor-panel :deep(.n-select),
.basic-grid .card-grid-item:last-child .editor-panel :deep(.n-base-selection) {
  max-width: 240px;
}
</style>
